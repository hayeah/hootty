...
//     libghostty snapshot
//   - sends the snapshot, then forwards live chunks
...
func (h *attachHandler) serveOne(conn io.ReadWriteCloser, bufrw *bufio.ReadWriter) error {
	...
	// Forward live chunks straight to the client. The snapshot above
	// reflects the emulator's state up to (but not including) any
	// chunk on liveCh, so there's no buffering phase to merge.
