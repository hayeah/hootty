...
// createSession is POST /sessions. It forks `supervise __supervise`
// directly from this process so the child supervisor is our
...
// supervisor's pid; the library handles the rest (drain children,
...
// spawnSupervise forks `supervise __supervise --state-dir <d>
...
func spawnSupervise(stateDir, key string, argv []string) (string, error) {
	...
	// Setsid: supervisor in its own session. See run.go for why.
