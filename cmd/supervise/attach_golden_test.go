...
import (
	...
	"github.com/hayeah/supervisor/internal/attachetest"
)
...
func TestAttachGoldenSnapshots(t *testing.T) {
	...
	t.Run("asciicast playback follows visible screen phase", func(t *testing.T) {
		rec, err := supervisor.NewRecorder(filepath.Join(t.TempDir(), "pty.cast"), 40, 8)
		if err != nil {
			t.Fatalf("NewRecorder: %v", err)
		}
		remote := attachetest.NewRemoteWithOptions(t, 40, 8, supervisor.WithRecorder(rec))
		local := attachetest.NewLocalTerminal(t, 40, 8)
