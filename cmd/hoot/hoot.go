...
// cmdHoot is the internal hootty process that holds the
...
func cmdHoot(args []string) error {
	fs := flag.NewFlagSet("__hoot", flag.ContinueOnError)
	...
	rest := fs.Args()
	if len(rest) == 0 {
		return errors.New("__hoot: missing command after --")
	}

	// Hootty stderr is the PTY slave; the library's slog
	// default would leak hootty-internal log lines into the
	// captured stream. Send slog to a file inside the state dir
	// instead.
	logPath := filepath.Join(*stateDir, *key, "hootty.log")
	...
	if master == nil {
		return errors.New("__hoot: PTY master not provided on fd 3 (invoked outside `hoot run`?)")
	}
	...
	// The state dir is created by the hootty library when it
	...
	rec, err := hootty.NewRecorder(filepath.Join(dir, "pty.cast"), cols, rows,
		hootty.WithRecorderCommand(strings.Join(rest, " ")),
		hootty.WithRecorderTitle(*key),
	)
	if err != nil {
		return fmt.Errorf("recorder: %w", err)
	}

	ptyImpl, err := hootty.NewLibghosttyPTY(master, cols, rows,
		hootty.WithRecorder(rec),
	)
	if err != nil {
