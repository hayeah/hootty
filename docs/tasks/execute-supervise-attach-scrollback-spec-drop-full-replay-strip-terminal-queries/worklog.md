---
status: done
section: Execute supervise-attach scrollback spec (drop full-replay, strip terminal queries)
slug: execute-supervise-attach-scrollback-spec-drop-full-replay-strip-terminal-queries
mode: worktree
spec: spec.md
created: 2026-05-02T15:15:56Z
---

> ## Execute supervise-attach scrollback spec (drop full-replay, strip terminal queries)
>
> ---
> status:
>   type: open
...
>
> Implement the spec at `/Users/me/Dropbox/notes/2026-05-02/supervise-attach-scrollback-spec_claude.md`. Read it end-to-end first — the "Strategy" section is the implementation plan, the "Verification plan" is the evidence list.
>
...
- [x] remove `--no-full-replay` flag and `replayMode` plumbing from `cmd/supervise/attach.go`
...
- 2026-05-02T15:27Z rebase conflict on master during lgtm. aborted — worktree is clean.
  ...
  cmd/supervise/attach.go and cmd/supervise/main.go also touched on master but auto-resolved; only README needs hand-resolution.
  
  commits that landed on master since you branched:
  - 3b18a3c "attach: switch default prefix + commands to mosh defaults" — README.md, cmd/supervise/attach.go, cmd/supervise/main.go
...
```
ok   github.com/hayeah/supervisor                       0.325s
ok   github.com/hayeah/supervisor/cmd/supervise         (cached)
