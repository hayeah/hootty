---
overview: Spec for fixing supervise's attach scrollback and the related "strange chars after tmux detach" bug. Covers root cause (doubled terminal-query responses), what the libghostty VT formatter actually emits (it includes scrollback — verified empirically), and a two-part fix.
repo: ~/github.com/hayeah/supervisor
tags:
  - spec
---

# Supervise: attach scrollback strategy

## Problem

Two related symptoms surfaced while playing with `supervise attach`:
...
Experiment source (drop into `~/github.com/hayeah/supervisor/` and `go test -run TestScrollbackExperiment -v .`):
...
If a client attaches while the child is on alt screen (tmux, vim, less, etc.), only alt-screen content is sent. When the child later exits alt, the client's terminal restores *its own* empty primary — supervise's recorded primary scrollback never reaches it.
...
Full analysis and the Option A/B/C tradeoff is captured in [`supervise-alt-screen-scrollback-gap_claude.md`](supervise-alt-screen-scrollback-gap_claude.md).
...
- `cmd/supervise/attach.go:77` — drop the `--no-full-replay` flag.
- `cmd/supervise/attach.go:95-97` — drop the mode toggle; always send `replayMode = "snapshot"`.
...
- Add a comment near `Snapshot()` documenting the alt-screen scrollback gap, pointing at `~/Dropbox/notes/2026-05-02/supervise-alt-screen-scrollback-gap_claude.md`.
...
1. `supervise attach <key>`, run `tmux attach`, do work, detach. No junk bytes at zsh prompt.
2. `supervise attach <key>` to a session with substantial history → scrollback is visible above the viewport (up to `max_scrollback`).
3. `supervise attach <key>` while child is in alt screen → repaint shows the alt screen, not stale primary history. (Known limitation: primary scrollback won't reappear when alt exits — see linked gap note.)
4. Detach + reattach mid-tmux session → tmux UI repaints, no probe leakage.

## Files touched (anticipated)

- `cmd/supervise/attach.go` — flip default `replayMode`.
