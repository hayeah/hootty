---
status: done
section: Remote attach over HTTP via supervise serve
slug: remote-attach-over-http-via-supervise-serve
mode: worktree
spec: spec.md
...
>
> `supervise attach` today only talks to a local `rpc.sock`. We want remote attach over HTTP against a `supervise serve` host (typically a Tailscale hostname + port — no auth required since Tailscale handles network ACLs).
>
> Target shape:
>
> ```
...
- [x] Drive-by: `supervise serve` `--addr`+`--port` → `--bind <host:port>` (serve.go, main.go usage, README, devport.toml)
- [x] Factor `runAttach` so the dialer is a parameter `func(ctx) (net.Conn, error)`; existing local path passes a unix dialer
- [x] Add `--host` flag + URL normalization (`host:port` | `http://…` | `https://…`) in `cmd/supervise/attach.go`
- [x] Add `handleAttachRaw` in `cmd/supervise/serve_bridge.go` (resolve → dialAttach(rpc.sock) → hijack → io.Copy×2), register route in `serve.go`
...
## Agent log
- 2026-05-02T12:15Z — rfc gate ticked by boss. Aligned Phase 2 todos with the final spec shape (server-side resolve, HTTP Upgrade/hijack, retry-forever, keystroke-wake). status: working. Starting with Step 0: `--bind` rename on `supervise serve`.
- 2026-05-02T12:08Z — Human said "use HTTP Upgrade/hijack instead of WebSocket". Updated spec: `attach-raw` is HTTP/1.1 Upgrade with token `supervise-attach/1`, bridge is a pure `io.Copy` relay, CLI reuses the existing local Upgrade dance with a swapped dialer. No `coder/websocket` import on the CLI side. Updated route description, URL section, Steps, and design note. Browser path (`/attach` WS) unchanged. Still blocked on rfc.
...
```
ok  	github.com/hayeah/supervisor
ok  	github.com/hayeah/supervisor/cmd/supervise   ← new TestHandleAttachRaw{RoundTrip,NotFound,Ambiguous}
ok  	github.com/hayeah/supervisor/internal/shortid
```

**`supervise serve` rename (Step 0) — `--bind <host:port>`:**

```
$ supervise serve --bind 127.0.0.1:20100 --state-dir /tmp/...
supervise serve: listening on http://127.0.0.1:20100 (state-dir=/tmp/...)
```

**Local attach (regression, exit 0):** stdout = `hello-from-supervise line2`, detach via `<prefix>d`.

**Remote attach over `--host` (exit 0):**

```
$ supervise attach --host 127.0.0.1:20100 smoke
hello-from-supervise
line2
...
```
$ supervise attach --host 127.0.0.1:20100 zzznotreal
supervise attach: no match for "zzznotreal"
exit=2
```
...
```
[supervise: disconnected: EOF]
[supervise: reconnecting in 1s — press any key to retry now]
[supervise: reconnecting in 2s — press any key to retry now]
[supervise: reconnecting in 1s — press any key to retry now]
```
...
- **One real bug found via smoke and fixed (`0115313`):** `<prefix>d` mid-backoff returned `ctx.Err()` up the call chain → "supervise attach: context canceled" exit 1. Mapped to clean exit 0 instead.
- **macOS `sun_path` 104-byte limit** bit the round-trip test (`t.TempDir()` paths exceed it). Fixed by adding a `/tmp`-rooted `shortTempDir` helper, mirroring the one in `supervisor_test.go`.
