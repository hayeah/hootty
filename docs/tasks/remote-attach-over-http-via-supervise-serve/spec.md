# Remote attach over HTTP via `supervise serve`

## Goal

Make `supervise attach` able to connect to a remote `supervise serve` host over HTTP/WebSocket, instead of only dialing a local `rpc.sock`. Target invocation:

```
supervise attach --host m4mini:20000 <short-id>
```
...
- New `--host` flag on `supervise attach`.
...
- Drive-by: collapse `supervise serve`'s `--addr` + `--port` into one `--bind` flag using Go's `net.Listen` `host:port` convention. Parallels the new `--host` on `attach` (one address-shaped flag per side).

Out of scope:

- TLS server cert pinning, auth tokens, anything that's not "trust the network".
- Any browser-side change. The existing `/sessions/{key}/attach` keeps its current shape (bridge synthesizes Hello at 80×24) for `supervisor-webui`.
...
- `cmd/supervise/attach.go` — extend with `--host` flag, remote dialer, reconnect loop. Local path stays the default.
- `cmd/supervise/serve.go` — register one new route; rename `--addr`+`--port` to `--bind <host:port>` (see "Serve `--bind`" below).
- `cmd/supervise/main.go` — usage block reflects `--bind`.
- `devport.toml` — update the `supervise serve` argv to `--bind 127.0.0.1:<port>`.
- `cmd/supervise/serve_bridge.go` — add `handleAttachRaw` (pure attachwire pass-through). The existing `handleAttach` is left untouched for browser callers.
- `cmd/supervise/serve_spawn.go` (or wherever short-id helpers live) — no change needed; client resolves via the existing `GET /sessions` endpoint.
- `README.md` — document `--host`.

### New server route

```
GET /sessions/{key}/attach-raw   →  HTTP/1.1 Upgrade: supervise-attach/1
                                    (hijacked TCP relay to rpc.sock /attach)
```

Wire shape:

- **HTTP/1.1 Upgrade**, not WebSocket. Same `supervise-attach/1` token the local `rpc.sock` `/attach` already uses. After the `101 Switching Protocols` exchange, the connection is raw bytes carrying attachwire frames in both directions. No second framing layer.
- The bridge implementation is a pure relay: resolve `{key}` → dial `rpc.sock` → perform the Upgrade dance against `rpc.sock` (existing `dialAttach`) → reply to the client with our own `101 Switching Protocols / Upgrade: supervise-attach/1` → `io.Copy` both directions until either side closes. The bridge does **not** parse attachwire; it just shovels bytes.
...
- **No extra dep on the client**: no `coder/websocket` import in `cmd/supervise/attach.go`.
...
- The existing `/attach` is a WebSocket bridge tailored for `supervisor-webui` (xterm.js sends JSON `{type:"resize"}`, bridge synthesizes `Hello{80x24}`). Conflating "browser WS bridge" and "CLI Upgrade pass-through" on one URL would mean branching on the request's `Upgrade` header inside the handler — possible but uglier than two routes that each do one thing.
...
The server already owns `supervisor.Store.Resolve` (handles exact-match + unique-prefix + ambiguous + missing). The client passes its raw user-typed argument straight through as the `{key}` path segment, and the bridge resolves on the server side.
...
A separate `GET /sessions` is still useful for `supervise list --host …` later, but it's not on the path for `attach`.
...
```
GET <scheme>://<host>/sessions/<key>/attach-raw     # with Upgrade: supervise-attach/1
```
...
- **reconnecting**. Termios stays in raw mode (we never restore between drops — restoring + re-raw would flicker and might race with the user's keystrokes). Backoff schedule: `1s, 2s, 4s, 8s, 16s, 30s, 30s, …`, capped at 30s. **No total cap — retry forever.** The user owns the lifecycle; only `<prefix>d` / SIGINT / SIGTERM ends a hung attach. Rationale: cover overnight-laptop and other long-outage scenarios where the supervisor outlives the network blip. On each retry tick, try the WS dial; on success, send a fresh Hello with current local cols/rows + the original `replay_mode`, transition to **connected**.
...
- **Default (`--full-replay`, the existing default)**: every reconnect issues `Hello{replay_mode:"full"}`. The supervisor streams the full pty.log again, repainting the screen — this is what makes the status-line hack work without a dedicated reserved row (see Status line below). This is the recommended mode for ssh-style sessions.
- **`--no-full-replay`**: every reconnect issues `Hello{replay_mode:"snapshot"}`. The supervisor sends a libghostty snapshot then streams. Faster on long-running sessions with huge pty.logs, but the disconnect line stays visible until the snapshot paint overwrites that row (mostly fine — alt-screen apps repaint everything).
...
```
\r\n\x1b[2m[supervise: disconnected, reconnecting in 2s — press any key to retry now]\x1b[0m\r\n
```
...
0. Drive-by rename: `supervise serve --addr/--port` → `--bind <host:port>`. Update `cmd/supervise/serve.go`, `cmd/supervise/main.go` usage, `README.md`, `devport.toml`. One commit.
1. Add `--host` flag parsing + URL normalization in `cmd/supervise/attach.go`. Pure flag plumbing, no behavior change yet (still local).
2. Add `handleAttachRaw` in `cmd/supervise/serve_bridge.go` and register the route in `serve.go`. Implementation: resolve key → call existing `dialAttach(ctx, sockPath)` which gives us an upstream `net.Conn` already past the rpc.sock 101 → hijack the client conn, write `101 Switching Protocols / Upgrade: supervise-attach/1`, then `io.Copy` both directions in two goroutines and wait for either side to close. ~30 lines. No attachwire parsing in the bridge.
...
7. Manual smoke + scripted test:
   - Local: `supervise serve --port 20000` + `supervise attach --host 127.0.0.1:20000 <short-id>`. Run vim, resize terminal, kill `serve`, restart it, watch the reconnect.
...
- Manual transcript captured into `tmp/<HHMMSS>_<ms>-attach-host-vim.txt`:
  1. spawn vim in a session via `supervise run`;
...
- 2026-05-02T11:12Z — Status-line approach: transient stderr line + replay-driven repaint. Alternatives considered:
  ...
  - **Cursor save / restore + clear-line on the last row** — `\x1b[s\x1b[<rows>;1H[supervise: …]\x1b[K\x1b[u`. Cleaner but still races with full-screen apps that own the cursor and assume their saved-cursor stack is private.
...
- 2026-05-02T11:40Z — Reconnect retries forever (no time cap), and any non-prefix keystroke during reconnecting wakes the backoff sleep for an immediate redial. Human-driven feedback. Reasons:
  - Primary use case is overnight-laptop / multi-hour outages where the supervisor outlives the network blip. A 5-min cap defeats that.
...
- 2026-05-02T11:30Z — `supervise serve --bind <host:port>` replaces `--addr` + `--port`. Human-driven feedback. Reasons:
...
- 2026-05-02T12:05Z — `attach-raw` uses HTTP/1.1 Upgrade + connection hijack, not WebSocket. Human-driven feedback. Reasons:
  - The local `rpc.sock` `/attach` already speaks HTTP Upgrade with token `supervise-attach/1`; reusing it across HTTP makes the bridge a pure TCP relay (`io.Copy` × 2, ~30 LOC) instead of a frame-aware translator.
  ...
  - Browser path is unaffected — the existing `/attach` WS bridge stays for `supervisor-webui`. `attach-raw` is the CLI/tooling route.
...
- 2026-05-02T11:50Z — Short-id resolved **server-side**: the bridge calls `store.Resolve({key})` on the path param, treating it as either a full key or a unique prefix. Human-driven feedback. Reasons:
  ...
  - One round-trip instead of two on first connect.
  - Single source of truth for the resolution rules — they can change in one place (`supervisor.Store.Resolve`) and both the local CLI and the remote CLI follow.
