---
status: done
section: Migrate supervisor termui+webui TS libs; add supervisor serve; wire devportv3
...
> ## Migrate supervisor termui+webui TS libs; add supervisor serve; wire devportv3
>
> ---
> status:
>   type: open
...
>
> Migrate the existing TS libs out of dotfiles into the supervisor repo, add a server that powers the example app, and stitch everything together with devportv3.
>
> Sources to migrate from:
> - `/Users/me/github.com/hayeah/dotfiles/libs/hayeah-go/supervisor/termui` — reusable terminal component
> - `/Users/me/github.com/hayeah/dotfiles/libs/hayeah-go/supervisor/webui` — example app consuming termui
>
> Targets (in `~/github.com/hayeah/supervisor`):
> - `@hayeah/supervisor-termui` — reusable component package
> - `@hayeah/supervisor-webui` — example app package
>
...
>
> New backend:
> - `supervise serve [--state-dir]` — HTTP API that routes to the individual `rpc.sock` of each process in the state-dir. This is what powers the example webui app. Reuse the per-session socket protocol; the server is a fan-out / multiplexer.
>
> Integration:
> - Pull everything together with `hayeah/devportv3`:
>   - proxy to vite live dev server (webui)
>   - proxy to `supervise serve` API
> - Evidence: use the `/browser` skill to take screenshots of the running webui talking to live supervisor sessions.
>
> - [ ] migrate termui and webui packages, scaffold with vp + /webui conventions
> - [ ] implement `supervise serve` and verify the webui talks to it
...
- [x] pnpm workspace at supervisor repo root (package.json, pnpm-workspace.yaml, .gitignore) — 29a222b
- [x] copy termui → packages/supervisor-termui, rename to @hayeah/supervisor-termui, build green — 3f6ca29
- [x] copy webui → packages/supervisor-webui, rename, fix workspace dep + 3 imports, build green — e38da7d
- [x] vite dev /preview smoke (mock data source) renders before any backend — tmp/preview-smoke.png

### Phase 2 — supervise serve (boss-doc box: implement supervise serve and verify)

- [x] go.mod: add github.com/coder/websocket — c646793
- [x] cmd/supervise/serve.go — flags, mux, GET /sessions, GET/DELETE /sessions/{key}, /healthz — 21bf44b
- [x] cmd/supervise/serve_spawn.go — POST /sessions (splitCmd + spawnSupervise), closeSession — 2a93db2
- [x] cmd/supervise/serve_bridge.go — handleAttach (WS↔attachwire), handleEvents (SSE proxy) — 47b9ec5
- [x] wire serve subcommand into main.go switch + usage() — 47b9ec5
- [x] curl/websocat smoke against a live supervise run — tmp/serve-smoke-transcript.txt

### Phase 3 — devportv3 wiring + evidence (boss-doc box: wire devportv3 + screenshots)

- [x] devport.toml composing vite + supervise serve under one proxy — 3a0e72b
...
- [x] update supervisor/README.md with serve + devport pointer — 3a0e72b
...
- 2026-05-02T10:33Z unblocked. wrote spec.md, seeded 13 todos across 3 phases. supervisor worktree leased at slot 000. starting phase 1 (TS migration).
- 2026-05-02T10:33Z phase 1.1 done — pnpm workspace scaffold landed (29a222b)
- 2026-05-02T10:38Z phase 1 complete — termui (3f6ca29) + webui (e38da7d) ported, /preview screenshot captured (tmp/preview-smoke.png) showing sessions sidebar + terminal pane against MockDataSource. starting phase 2 (supervise serve).
- 2026-05-02T10:44Z phase 2 complete — supervise serve landed across 4 commits (c646793 21bf44b 2a93db2 47b9ec5); GET/POST/DELETE /sessions and WS /attach all proved against a live supervise run; transcript at tmp/serve-smoke-transcript.txt. starting phase 3 (devport.toml + browser screenshot).
...
```
$ pnpm --filter @hayeah/supervisor-termui build
ESM dist/index.js 5.32 KB
DTS dist/index.d.ts 1.92 KB

$ pnpm --filter @hayeah/supervisor-webui build
...
This proves the import rewrite (`@hayeah/termui` → `@hayeah/supervisor-termui` in 3 webui files) didn't break the workspace dep. The webui's `LiveDataSource` already targets `/api/sessions{,/:key{,/state,/events,/attach}}`, which is exactly the route shape phase 2's `supervise serve` will expose.

Commits: 29a222b (workspace) · 3f6ca29 (termui) · e38da7d (webui).

### Phase 2 — supervise serve

`go build ./cmd/supervise` and `go test ./...` both clean.
...
- `DELETE /api/sessions/api2` → 204; subsequent list shows it gone (the supervisor exited cleanly).
