# spec.md — extract supervisor library; pty+libghostty (drop tmux spawn)

## Goal

Lift the supervisor code at `~/github.com/hayeah/dotfiles/libs/hayeah-go/supervisor` into a new standalone repo `github.com/hayeah/supervisor`. Trim it to the bits the task names: a process supervisor that spawns a child on a libghostty-backed PTY, tees the raw output for replay, and exposes terminal state through an embeddable `http.ServeMux` (text / HTML / raw VT). Ship a small `supervise` example CLI that serves the mux on a unix socket.

In scope:
- New repo `hayeah/supervisor` (module `github.com/hayeah/supervisor`).
...
- Example `supervise` CLI: launches a process with a target dir, serves the mux on `<dir>/rpc.sock`.
- Drop the tmux spawn backend (`pty_tmux.go`, `tmux.go`, the TmuxSpawn config). Drop unused webui/termui/ptyclient subpackages — those are not the supervise example shape the task describes.
...
```
hayeah/supervisor/
  go.mod                          # module github.com/hayeah/supervisor
  README.md
  Makefile                        # libghostty-vt build/PKG_CONFIG glue
  supervisor.go                   # Runner, Mux(), Run(ctx)
  supervisor_iface.go             # Supervisor interface
  ...
  cmd/supervise/
    main.go run.go supervise.go service.go common.go
```

Key flow (matches existing dotfiles design — emulator pattern):
1. `supervise <args> -- <cmd>` opens a PTY pair via `creack/pty`, sets winsize, forks itself with `__supervise` subcommand: stdio = slave, ExtraFiles[0] = master, Setsid (no controlling tty in parent).
2. `__supervise` picks master off fd 3, builds `LibghosttyPTY`, builds the `Runner`, calls `runner.Run(ctx)`.
...
- Init `~/github.com/hayeah/supervisor` as a git repo, master branch, baseline empty commit.
- `boss checkout` it into the workspace.
- Create `go.mod` for `github.com/hayeah/supervisor`, add deps (`creack/pty`, `mitchellh/go-libghostty`, `golang.org/x/term`).
- Add Makefile + README skeleton.

Phase 2 — port library bits (no PTY yet)
- Copy `atomic.go state.go store.go flock.go poll.go tailfile.go signals.go eventbus.go service.go supervisor_iface.go supervisor.go` (and their tests) into the new repo. Strip TmuxSpawn references.
- Update import paths to `github.com/hayeah/supervisor`.
- Drop the `Plugin`/`KillOnExit`/`Spawn` shape from old `SupervisorConfig`. Keep the four-field shape that the dotfiles version already converged on (StateDir, Key, Service, PTY).
...
Phase 5 — `supervise` CLI
- `cmd/supervise/{main,run,supervise,service,common}.go`. Two subcommands: `supervise run -- <cmd>` (parent) and `supervise __supervise --key K --state-dir D -- <cmd>` (child fork). Shape mirrors dotfiles' ptydemo, just renamed and trimmed.
...
- E2E smoke: `supervise run -- bash -c 'echo hello; sleep 1'` in detached mode (no attach), then curl `/pty/text`, `/pty/html`, `/pty/vt`, `/pty/stream` over the unix socket, expect "hello" in each.

Phase 7 — README + finalize
- README with: motivation, library API, build setup (libghostty-vt prereqs), `supervise` CLI usage, future-work pointers (attach CLI / recorded log replay).

## Verification

- `go test ./...` from the repo root passes (with `PKG_CONFIG_PATH` exported via Makefile target `make test`).
- `make build` produces `bin/supervise`.
- E2E transcript captured under `tmp/`:
  - Start: `supervise run --state-dir <tmp> --key demo -- bash -lc 'echo hello-world; printf "\\x1b[1mbold\\x1b[0m\\n"; sleep 30' &`
...
- 2026-05-02T06:40Z — Worker stderr leaks into the captured PTY stream because the worker process inherits stdio = PTY slave; the supervisor lib's default `slog.Default()` writes to stderr → the slave → the master → libghostty Terminal → `/pty/text` shows the supervisor's own log lines.
  - Caught at e2e smoke: `acquired flock supervisor=demo` appeared in the first `/pty/text` capture.
  - Fix: in `cmd/supervise/supervise.go` (the `__supervise` entry), redirect `slog.Default()` to `<state>/<key>/supervisor.log` before constructing the Runner. Falls back to `io.Discard` if the file can't be opened.
