---
status: done
section: Extract supervisor library with pty+libghostty (drop tmux spawn)
slug: extract-supervisor-library-with-pty-libghostty-drop-tmux-spawn
mode: worktree
spec: spec.md
created: 2026-05-02T06:20:48Z
---

> ## Extract supervisor library with pty+libghostty (drop tmux spawn)
>
> ---
> status:
>   type: open
...
>
> Example app — a simple `supervise` CLI:
...
>
> - [ ] extract supervisor lib to standalone repo, swap tmux spawn for pty+libghostty, ship the example `supervise` CLI

## Todos
<!-- Finer-grained than the boss-doc top-level checkboxes. Tick off as you go. -->

### Phase 1 — repo skeleton
- [x] init `~/github.com/hayeah/supervisor` repo, master baseline, boss checkout
- [x] write go.mod, Makefile, README skeleton

### Phase 2 — port library plumbing
- [x] copy atomic/state/store/flock/poll/tailfile/signals/eventbus/service/supervisor_iface/supervisor + tests
- [x] strip TmuxSpawn/Plugin/KillOnExit; rewrite imports to `github.com/hayeah/supervisor`
...
- [x] cmd/supervise/{main,run,supervise,service,common}.go

### Phase 6 — verification
- [x] unit tests for libghostty PTY (feed, dump, html, vt, send-keys literal, recorder roundtrip, key parser)
- [x] e2e smoke: `supervise run -- bash -c '...'` + curl all 7 endpoints
- [x] populate `## Evidence`

### Phase 7 — README + finalize
- [x] README: motivation, API, build setup, supervise usage, future work (covered in repo README at phase 1; verified accurate)

## Agent log
- 2026-05-02T06:24Z spec written; new repo hayeah/supervisor inited+checked out; phase 1 todo seeded
...
- 2026-05-02T06:41Z all phases done; e2e smoke captured at tmp/133940_813-e2e.txt; status -> done (ebe5bca, branch extract-supervisor-library-with-pty-libghostty-drop-tmux-spawn)

## Boss log

## Evidence

New repo: `~/github.com/hayeah/supervisor` (worktree at `repos/github.com/hayeah/supervisor`, branch `extract-supervisor-library-with-pty-libghostty-drop-tmux-spawn`).

### Build + tests

```
$ cd ~/github.com/hayeah/supervisor/.worktrees/000
$ make build
go build -o bin/supervise ./cmd/supervise
$ make test
go test ./...
ok  	github.com/hayeah/supervisor	0.777s
...
```
$ ./bin/supervise run --state-dir /tmp/sv.fRuJ3Q --key demo -- \
    bash -lc 'echo hello-world; printf "\x1b[1mbold\x1b[0m\n"; \
              printf "\x1b[31mred\x1b[0m\n"; sleep 30'
supervise: session "demo" started (state-dir=/tmp/sv.fRuJ3Q)
demo

$ ls /tmp/sv.fRuJ3Q/demo/
pty.log  rpc.sock  state.json  supervisor.log
...
# Send Ctrl-C: child exits, supervisor publishes exit state.
...
- live stream + raw input: `/pty/send-keys` encodes Ctrl-C through libghostty's KeyEncoder, the child receives SIGINT, the supervisor publishes `state=exited` in `state.json` and tears down `rpc.sock`.
