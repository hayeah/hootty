...
>
> The current `C-b` default for `hoot attach` collides with tmux's prefix — bad UX when attaching into a shell that runs tmux. Switch to mosh's convention wholesale:
>
> - **Prefix**: `Ctrl-^` (0x1e), not `C-b`.
> - **Commands inside an attach** (mosh vocabulary):
>   - `Ctrl-Z` → suspend the local `hoot attach` process (SIGTSTP, fg to resume; same as mosh).
...
- [x] flip default prefix to `C-^` (0x1e) and update FSM command vocabulary (`.`=detach, `^`=literal, `Ctrl-Z`=suspend, `?`=help) in `cmd/hoot/attach.go`
- [x] refresh in-attach help text in mosh style
- [x] update `cmd/hoot/main.go` top-level usage text
...
$ go build ./...
$ go test ./...
ok  	github.com/hayeah/hootty	0.961s
ok  	github.com/hayeah/hootty/cmd/hoot	0.512s
?   	github.com/hayeah/hootty/internal/attachwire	[no test files]
ok  	github.com/hayeah/hootty/internal/shortid	1.165s
```
...
### New `hoot attach` usage

```
$ hoot attach
usage: hoot attach [flags] <id-or-prefix>
...
```
hoot attach commands: "." detach, "^" literal prefix, "Ctrl-Z" suspend, "?" help
```
...
- No friction. Single localized change set; no shared-protocol fallout (prefix logic is purely client-side in `cmd/hoot/attach.go`).
