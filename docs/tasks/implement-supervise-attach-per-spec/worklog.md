---
status: done
section: Implement supervise attach per spec
slug: implement-supervise-attach-per-spec
mode: worktree
spec:
created: 2026-05-02T09:10:49Z
---

> ## Implement supervise attach per spec
>
> ---
> status:
>   type: open
> ---
>
> Implement `supervise attach` in `~/github.com/hayeah/supervisor` per the spec at:
>
> - `/Users/me/Dropbox/notes/2026-05-02/supervise-attach-spec_claude.md` (the design — read this fully, it's the source of truth)
>
...
> - new CLI: `supervise attach [--state-dir D] [--no-full-replay] [--prefix-key K] <id-or-prefix>`
> - prefix-resolves the session id (3-char min, ambiguity errors out — same as `supervise resolve`)
> - single rpc.sock connection upgraded into a small TLV binary protocol (`Hello` / `Input` / `Output` / `Size`)
> - new file layout per spec: `cmd/supervise/attach.go`, `internal/attachwire/wire.go`, `supervisor/attach.go`, `supervisor/attach_handler.go`
...
>
> - [ ] implement supervise attach per spec, including server-side AttachSet/handler, attachwire framing, client CLI with prefix-key state machine, full-replay cutover, min-wins winsize negotiation, and the evidence list above
...
- [x] write `supervisor/attach.go` — AttachSet (min-wins winsize negotiation)
- [x] write `supervisor/attach_handler.go` — HTTP upgrade → binary protocol on `/attach`, replay+live cutover
- [x] register `/attach` route in PTY's RegisterRoutes
- [x] write `cmd/supervise/attach.go` — client CLI w/ raw termios, prefix-key state machine, SIGWINCH coalescer, exit codes
- [x] wire `attach` into `cmd/supervise/main.go` usage + dispatch
...
- [x] update README with `supervise attach` docs
...
All e2e checks ran against `bin = go build -o /tmp/supervise-test ./cmd/supervise`. Tests passed: `go test ./...` → `ok github.com/hayeah/supervisor`, `ok internal/shortid`.
...
```
$ /tmp/supervise-test attach --prefix-key foo bar
supervise attach: prefix-key: unrecognized form "foo"          → exit 2

$ /tmp/supervise-test attach --prefix-key 0x41 bar
supervise attach: prefix-key "0x41" is a printable byte; printable prefixes silently eat user typing  → exit 2

$ /tmp/supervise-test attach --prefix-key 'a' bar
supervise attach: prefix-key "a" is a printable byte; printable prefixes silently eat user typing  → exit 2
```
...
- **`internal/attachwire` is import-restricted** to the supervisor module. The standalone test driver under `tmp/attach_driver.go` had to inline the framing (≤30 lines). Acceptable; the driver is throwaway test glue.
