---
status: done
section: Remove PTY interface and drop SendKeys from supervisor lib
slug: remove-pty-interface-and-drop-sendkeys-from-supervisor-lib
mode: worktree
spec:
created: 2026-05-02T07:42:45Z
---

> ## Remove PTY interface and drop SendKeys from supervisor lib
>
> ---
> status:
>   type: open
> ---
>
> In `~/github.com/hayeah/supervisor`, drop the `type PTY interface { ... }` abstraction now that tmux is no longer a spawn backend — there's only one implementation (`LibghosttyPTY`). Use the concrete type directly throughout the library and CLI.
>
...
>
> Update tests, README, and the `supervise` example CLI accordingly. Keep the e2e smoke working without the send-keys step.
>
> - [ ] drop PTY interface, remove SendKeys + /pty/send-keys + key-name parser, update tests + README + supervise CLI
...
- [x] switch `SupervisorConfig.PTY` / `Runner.PTY()` / `Supervisor.PTY()` to concrete `*LibghosttyPTY`; drop the RegisterRoutes type-assertion (call directly)
- [x] update `supervisor_test.go`: replace `fakePTY` with a real `*LibghosttyPTY` from `pty.Open()`
...
- Decision: `SupervisorConfig.PTY` is now `*LibghosttyPTY` (concrete),
  not an interface — matches the section directive. The fakePTY stub
  in `supervisor_test.go` was the only thing the interface bought us
...
PASS
ok  	github.com/hayeah/supervisor	0.295s
```

### `go vet ./...` and `go build`
Both clean. `bin/supervise` rebuilds with no diagnostics.

### `supervise` CLI e2e smoke (no send-keys, raw input only)
```
$ ./bin/supervise run --state-dir $SD --key smoke -- bash -c 'echo hello-from-child; sleep 5'
supervise: session "smoke" started (state-dir=/tmp/svsmoke.ePbJ)
smoke
...
{"supervisor":{"key":"smoke","pid":23781,...},
...
$ # Child got SIGINT, supervisor exited, rpc.sock is gone:
$ curl -s --unix-socket $SD/smoke/rpc.sock http://x/state
...
extra real PTY pair per test that needed a Supervisor (~5), which is
trivial.
