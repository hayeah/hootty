...
Move the existing TypeScript dashboard out of `dotfiles/libs/hayeah-go/hootty/{termui,webui}` into the `~/github.com/hayeah/hootty` repo as a pnpm workspace, port the dotfiles `ptydemo serve` API as a new `hoot serve` subcommand, and wire the whole thing together with a `devport.toml` that fronts vite + the API on a single proxy. End state: `cd repos/.../hootty && devport up` brings up a working dashboard at one URL with live hootty sessions; `/browser screenshot` proves it.

Out of scope: any new termui/webui features, redesigning the wire protocol, changing the hootty library itself.

# Architecture

## Repo layout (target hootty repo, post-migration)

```
github.com/hayeah/hootty/
  go.mod / go.sum / go.work          # existing
  *.go                                # hootty library + attach handler (existing)
  cmd/hoot/
    ...
    serve_spawn.go                    # NEW: createSession + spawnHoot + splitCmd
...
| POST   | `/sessions`                   | `{cmd?, argv?, key?}` → spawn `hoot __hoot --state-dir <d> --key <k> -- <argv...>`, wait for sock  |
| GET    | `/sessions/{key}`             | state.json                                                                                                    |
| DELETE | `/sessions/{key}`             | SIGTERM `state.Hootty.PID`, 204                                                                           |
...
Backend side (target hootty `attach_handler.go` over `rpc.sock`):
...
Decision: bridge talks `attachwire` directly rather than going through the older `/pty/{stream,input,resize}` REST routes. That keeps the contract narrow (one hootty endpoint instead of three) and matches what `hoot attach` already does. The `ptyclient` package from dotfiles is *not* ported.

### Spawning hoottys from POST /sessions

Reuses the `pty.Open()` + fork-self pattern from the dotfiles `serve_spawn.go`, translated for the target binary's flag layout (`hoot __hoot --state-dir <d> --key <k> -- <argv...>` instead of `ptydemo hoot ...`). Returns 201 with `{...StateFile, alive:true, socket_path}` once `rpc.sock` appears (3s timeout).
...
- pnpm workspace at hootty repo root: `packages: ["packages/*"]`.
...
Add `devport.toml` at the hootty repo root:

```toml
tmux_session = "hootty-dashboard"
...
1. Set up pnpm workspace at hootty repo root (`package.json`, `pnpm-workspace.yaml`, `.gitignore` for `node_modules/`).
...
7. Implement `cmd/hoot/serve_spawn.go` — `splitCmd`, `createSession`, `spawnHoot` (translated to `hoot __hoot` argv), `closeSession`.
...
13. Update `hootty/README.md` with a paragraph on `hoot serve` + a pointer to `devport up` for the example dashboard.

# Verification

- `pnpm --filter @hayeah/hootty-termui build` and `pnpm --filter @hayeah/hootty-webui build` both succeed.
- `go test ./...` in the hootty repo still passes.
...
None right now — the dotfiles ptydemo serve gives a near-line-for-line reference, the only translation work is the WS↔attachwire bridge (since the target hootty repo doesn't have the older `/pty/*` REST routes that `ptyclient` was built around — wait, it actually does: `pty_libghostty.go` mounts `/pty/{text,html,vt,stream,input,resize}`. So bridging via `ptyclient` is also viable — but I'm sticking with `/attach` because it's the supported public protocol and the legacy routes might be retired).
...
- 2026-05-02T10:35Z — Picked `/attach` (HTTP Upgrade + attachwire) over the older `/pty/{stream,input,resize}` REST routes for the WS bridge.
  - Both endpoints exist in the target hootty repo (saw both in `pty_libghostty.go:410-422`). The ptydemo reference impl in dotfiles uses the `/pty/*` flavor via `ptyclient`.
...
- 2026-05-02T10:35Z — Putting termui+webui under `packages/` rather than at repo root.
  - Source repo (dotfiles) had them flat at `hootty/termui/` and `hootty/webui/`. The target repo is a single Go module at root with many `*.go` files; mixing a `termui/` and `webui/` dir at root reads as more confusing than a `packages/` subtree.
